...
const ExpertApplicationContainer = () => {
    ...
    useEffect(()=>{
        axios.get(`http://49.50.163.215/api/admin/requested`,null,headers)
        .then((res)=>{
    ...
    return (
        <div className="ExpertApplicationContainer">
            <ExpertApplication userName={requested[0].userName} job={requested[0].job} company={requested[0].company} education={requested[0].education} uid={requested[0].uid}/>
            <ExpertApplication />
            <ExpertApplication />
            <ExpertApplication />
            {/* 페이지네이션 구현 하기 */}
